...
import { analyzeTranscriptDetailed } from '@/lib/gemini/detailed-analyzer';
...
export async function POST(request: NextRequest) {
    try {
        ...
        // Step 4: Detailed analysis with Gemini
        const analysis = await analyzeTranscriptDetailed(
            transcript,
            videoInfo.duration,
            videoInfo.title,
            url
        ...
        if (analysis.crucialParts.length === 0 && analysis.clippingIdeas.length === 0) {
            return NextResponse.json(
                { success: false, error: 'Could not generate analysis for this video.' },
                { status: 400 }
            );
        }
